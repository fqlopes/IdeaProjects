...
public class ContactData {
    ...
    public static List<Contact> getData (String type){
        ...
        while (scanner.hasNext()){
            ...
            if (type.equals("phone")){
                dataList.add(new Contact(data[0],String.valueOf(data[1])));
    ...
    public static List<Contact> getAllData (){
        ...
        List<Contact>dataList = new ArrayList<>();
        Scanner scanPhones = new Scanner(phoneData);
        Scanner scanEmails = new Scanner (emailData);

        while(scanPhones.hasNext()){
            String[] data = scanPhones.nextLine().split(",");
            Arrays.asList(data).replaceAll(String::trim);
            dataList.add(new Contact(data[0],String.valueOf(data[1])));
        }

        while(scanEmails.hasNext()){
            String[] data = scanEmails.nextLine().split(",");
            Arrays.asList(data).replaceAll(String::trim);
            dataList.add(new Contact(data[0],data[1]));
        }
