...
//            statement.execute(alterString);
    ...
    private static void addOrders(Connection conn, List<Order> orders) {
        ...
        String insertDetail = "INSERT INTO storefront.order_details (order_id, item_description, quantity) " +
                "VALUES (?, ?, ?)";

        try (PreparedStatement psOrder = conn.prepareStatement(insertOrder, PreparedStatement.RETURN_GENERATED_KEYS);
             PreparedStatement psDetail = conn.prepareStatement(insertDetail, PreparedStatement.RETURN_GENERATED_KEYS)) {
