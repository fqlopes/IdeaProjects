...
class RecursiveSumTask extends RecursiveTask<Long> {
    ...
    protected Long compute() {
        ...
        if((end - start) <= (numbers.length / division)) {
            ...
            int mid = (start + end) / 2;
            RecursiveTask<Long> leftTask = new RecursiveSumTask(numbers, start, mid, division);
            RecursiveTask <Long> rightTask = new RecursiveSumTask(numbers, mid, end, division);
            leftTask.fork();
            rightTask.fork();
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Threaded Pool Sum = " + taskSum);
