...
public class Main {
    public static void main(String[] args) {
        ...
        String header = """
                ...
                Experienced,Couse Code,Engagement Month,Engagement Year,\
                Engagement Type""";
            ...
            for (Student student : students) {
                for (var record : student.getEngagementRecords()) {
                    ...
                    writer.printf("%-12d%-5s%2d%4d%3d%-1s".formatted(
                            student.getStudentId(),student.getCountry(),
                            student.getEnrollmentYear(), student.getEnrollmentMonth(),
                            student.getEnrollmentAge(), student.getGender()
                    ));
                    writer.printf("%-1s", (student.hasExperience() ? 'Y' : 'N'));
                    writer.format("%-3s%10.2f%-10s%-4s%-30s",
                            recordData[7], //courseCode
                            student.getPercentComplete(recordData[7]),
                            recordData[8], //engagementMonth
                            recordData[9], //engagementYear
                            recordData[10]); //engagementType
                    writer.println();
                }
            }
        } catch (IOException e) {
